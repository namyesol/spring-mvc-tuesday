    ...
    <div class="flex justify-between px-4 mx-auto max-w-screen-xl ">
        <article class="mx-auto w-full max-w-2xl format format-sm sm:format-base lg:format-lg format-blue dark:format-invert">
            <header class="mb-4 lg:mb-6 not-format">
                <h1 class="mb-4 text-3xl font-extrabold leading-tight text-gray-900 lg:mb-6 lg:text-4xl dark:text-white">${storageObject.name}</h1>
                <!-- Author Profile -->
                <address class="flex items-center mb-6 not-italic">
                    <div class="inline-flex items-center mr-3 text-sm text-gray-900 dark:text-white">
                        <div>
                            <a href="#" rel="author" class="text-xl font-bold text-gray-900 dark:text-white">${storageObject.memberId}</a>
                            <p class="text-base font-light text-gray-500 dark:text-gray-400">작성 시각<time pubdate datetime="2022-02-08" title="February 8th, 2022"> : ${storageObject.createdAt}</time></p>
                        </div>
                    </div>
                </address>
                <!-- Author Profile -->
                <!-- AttachFiles -->
                <c:if test="${not empty storageObject.files}">
                    <div class="mb-4 flex flex-col">
                        <c:forEach var="file" items="${storageObject.files}">
                            <spring:url var="fileLink" value="/downloads/${storageObject.id}/${file.originalFileName}" htmlEscape="true"/>
                ...
                <!-- AttachFiles -->
                <!-- ImageFiles -->
                <c:if test="${not empty storageObject.imageFiles}">
                <div class="mb-4">
                    <c:forEach var="image" items="${storageObject.imageFiles}">
                        <spring:url var="imageSrc" value="/images/${image.storeFileName}" htmlEscape="ture"/>
                        <figure class="max-w-lg">
                            <img class="h-auto max-w-full rounded-lg" src="${imageSrc}" alt="image description">
                            <figcaption class="mt-2 text-sm text-center text-gray-500 dark:text-gray-400">${image.originalFileName}</figcaption>
                        </figure>
                    </c:forEach>
                </div>
                </c:if>
                <!-- ImageFiles -->
                <p class="mb-4 text">${storageObject.description}</p>
